...
- The project is generated by **XcodeGen** from `project.yml`; add target membership there, or place app files under the globbed `Passo/` source path, then run `xcodegen generate` when regenerating the project.
- No package dependencies. CI runs iOS build checks, and `PassoTests` / `PassoUITests` targets exist.
...
- **票据 tab**: event tickets only (`!isCard && !isInArchive`), with in-tab 即将 / 全部 segmented control. 全部 mode adds search and per-type filters.
- **卡包 tab**: long-lived membership cards (`ticketType == .member`) in a grid.
- **Imports**: launched from a `+` menu as 从相册导入 / 扫描条码 / 手动新建, presenting `PhotoImportView`, `ScanView`, or an empty-`Ticket` `RecognitionConfirmView`. `importPreferredType` defaults card-wallet imports to `.member`.
- **已归档** (`ArchiveView`): unified archive for expired, used, and manually archived tickets and cards, reachable from both 票据 and 卡包 tabs.
- **Single model** (`Models/Ticket.swift`): `Ticket` is the only SwiftData `@Model`. `TicketType` persists as `ticketTypeRaw: String`; computed `ticketType` rehydrates from `rawValue`. `isCard`, `isInArchive`, `canRestore`, and `isExpiringSoon` drive bucketing.
- **Default expiry rules** live in `Ticket.defaultExpiry(for:eventDate:)`.
- **Source layout**: app code lives under `Passo/`, Share Extension code under `PassoShareExtension/`, tests under `PassoTests/` and `PassoUITests/`.
...
- `Docs/`: durable UI and development guidelines.
...
3. `PhotoImportView` and `ScreenshotImportView` run image analysis through `TicketParser.analyzeImage(...)`.
4. `TicketParser.parse(barcodeValue:ocrText:)` and image analysis create best-effort `Ticket` values.
5. `RecognitionConfirmView` lets the user edit fields before persistence and Wallet signing; `.confirm` mode supports add-to-Wallet and save-only paths, while `.edit` mode saves an existing ticket.
6. Share Extension import writes an App Group payload, opens `passo://import`, and lets the main app consume the payload through `ShareImportService`.

Parsing must be local, deterministic, and recoverable. Worst case should produce an editable `.generic` ticket.

## PassKit And Signing

The client pipeline is implemented: `PassBuilder` builds `pass.json`, `SigningService` posts it to a signing node, and `WalletPresenter` wraps `PKAddPassesViewController`.

Pass signing still requires remote infrastructure. `NodePreference` is persisted via `@AppStorage("signingNodePreference")`.
...
Client-side scan, parser, signing request, Wallet presentation, and save-only fallback are implemented. Remaining gaps depend on external setup:

- Signing nodes are placeholders; no production signing server is deployed.
- Team ID / Pass Type ID needs build-setting configuration for real device signing.
- `ProUpgradeSheet` is placeholder UI until App Store Connect products exist.
- Share Extension embedding and iCloud sync need device / project verification.
- `ModelConfiguration` currently uses `cloudKitDatabase: .none`; the iCloud toggle is not a live CloudKit switch.
...
`StoreService` is the StoreKit 2 entitlement source and syncs Pro status back to `UserDefaults`. Free import quota is centralized in `StoreService.remainingFreeImports(...)` and `StoreService.isAtFreeImportLimit(...)`.
